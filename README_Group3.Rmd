...
We also model the effect of the program on test scores making it either biased or non-biased. Effect will be saved as scalar 'b_treat'. We are basically looking at the actual true effect side-by-side with the treatment effect (b_treat). 


## Power calculations and results.do Description

results.do sets the working directory, a randomization seed, and then loads the schools program defined in simulate.do. 

Then it runs the program with different ICC values (0.01, 0.25, 0.5, 0.75, and 0.99), running each 100 times and storing the results in the results matrix. It runs the program over effect size 'k' (5, 10, 15, 20). We also apply a p-value here to determine if to reject or fail to reject the null hypothesis. Our local alpha is set to 0.05. We reject the null hypothesis if the p-value is less than 0.05, which will be 'reject = 1'.
...
Clearly, a larger ICC corresponds to a larger design effect, as we would expect from their theoretical relationship. This implies that the sample size has to be increased when there is high variance between clusters in order to achieve the same minimal detectable effect. We also see values close in range with the estimated treatment effect and the true effect that we generated. 
