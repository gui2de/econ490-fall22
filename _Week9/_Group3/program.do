...
**** (3) Defining score variable based on treatment status
	...
	if treat == 0 & frpl == 0 {
		gen score = round(70 + 10*(u + e))
		// 
	}
	if treat == 0 & frpl == 1 {
		gen score = round(60 + 10*(u + e)) // assuming low-income students have lower test scores
	}
	if treat == 1 & frpl == 0 {
		gen score = round(70 + `effect_size' + 10*(u + e))
	}
	if frpl_treat == 1 {
		gen score = round(60 + 1.5*`effect_size' + 10*(u + e)) // assuming low income students see a larger benefit from program
	}
