...
prog def schoolpower, rclass
	...
	// generating actual intracluster correlation in the data
		...
		* for student i, school j: Y_ij = \mu + u_j + e_ij, where \mu = 70
		...
		gen compl_rate = round(runiform(0,1)) // rate of grade completion/graduation after experiment
		gen tchr_sat = round(rnormal(1,5))
		replace tchr_sat = 0 if tchr_sat < 0 // teachers satisfaction and evaluation of student
		* add a treatment status variable "treat"
		* add a baseline test score variable "basescore"
if "`biased'" == "" {mixed score treat basescore absences disc bmi frpl compl_rate tchr_sat || school: // non-biased, with previous eligibility for FRPL
}
